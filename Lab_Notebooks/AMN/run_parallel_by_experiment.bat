@echo off
REM Batch script to run the parallel dataset generation by experiment
REM This activates the conda environment and runs the Python script
REM 
REM Usage:
REM   run_parallel_by_experiment.bat [size_i] [reduce]
REM 
REM Arguments:
...
REM 
REM Examples:
REM   run_parallel_by_experiment.bat          - Use defaults (size_i=1, reduce=true)
REM   run_parallel_by_experiment.bat 10       - Use size_i=10, reduce=true
REM   run_parallel_by_experiment.bat 10 false - Use size_i=10, reduce=false

echo Activating conda environment...
call conda activate C:\Users\rh2310\projects\amn_release\.env

echo Running parallel dataset generation by experiment...

REM Build the command with optional arguments
set CMD=python build_dataset_parallel_by_experiment.py
