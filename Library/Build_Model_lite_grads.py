...
def Gradient_Descent(V, Vin, Vout, parameter, mask,
    ...
    # GD loop - Initialize history tracking
    Loss_mean_history, Loss_std_history, diff = [], [], 0 * V
    grad_history = {'timestep': [], 'grad_norm': [], 'diff_norm': [], 'V_norm': [],
    ...
    for t in range(1, parameter.timestep+1):  # Update V with GD
        ...
        if history:
            ...
            # Track gradients and norms per timestep
            grad_history['timestep'].append(t)
            grad_history['grad_norm'].append(float(tf.norm(dL).numpy()))
            grad_history['diff_norm'].append(float(tf.norm(diff).numpy()))
            grad_history['V_norm'].append(float(tf.norm(V).numpy()))
            
            # Track individual loss components
            if Vout.shape[0] > 0:
                L1, _ = Loss_Vout(V, parameter.Pout, Vout)
                grad_history['L1_loss'].append(float(tf.reduce_mean(L1).numpy()))
            else:
                grad_history['L1_loss'].append(0.0)
            L2, _ = Loss_SV(V, parameter.S)
            L3, _ = Loss_Vin(V, parameter.Pin, Vin, parameter.mediumbound, parameter)
            L4, _ = Loss_Vpos(V, parameter)
            grad_history['L2_loss'].append(float(tf.reduce_mean(L2).numpy()))
            grad_history['L3_loss'].append(float(tf.reduce_mean(L3).numpy()))
            grad_history['L4_loss'].append(float(tf.reduce_mean(L4).numpy()))
            
            if verbose and (np.log10(t) == int(np.log10(t)) \
                            or t/1.0e3 == int(t/1.0e3)):
                print('QP-Loss', t, Loss_mean, Loss_std, 'GradNorm:', grad_history['grad_norm'][-1])

    return V, Loss_mean_history, Loss_std_history, grad_history
...
def QP_layers(inputs, parameter, targets = np.asarray([]).reshape(0,0),
    ...
    V, Loss_mean, Loss_std, grad_history = Gradient_Descent(V0, Vin, Vout, parameter, mask,
                             trainable=trainable, history=history, verbose=verbose)
    outputs = output_AMN(V, Vin, V0, parameter, verbose=verbose) 

    return outputs, Loss_mean, Loss_std, grad_history
...
def AMN_QP(parameter, trainable=True, verbose=False):
    ...
    inputs = Input(shape=(input_dim,))
    outputs, loss_h, loss_std_h, grad_h = QP_layers(inputs, parameter,
                              trainable=trainable,
                              history=False,
                              verbose=verbose)
    # Compile
...
def LP(V, M, b_int, b_ext, parameter, verbose=False):
    ...
    def print_vals(V, name):
        print(f'max abs {name} and the corresponding index', tf.reduce_max(tf.abs(V)).numpy(), tf.argmax(tf.reshape(tf.abs(V), [-1])).numpy())
    ...
    # print_vals(dV1, 'dV1 after P')
    dV2 = tf.linalg.matvec(S_int, V) # dV2 = Q(S_intV - b_int)
    # print_vals(dV2, 'dV2')
    dV2 = dV2 - b_int   
    # print_vals(dV2, 'dV2 after b_int')
    dV2 = tf.linalg.matvec(Q,dV2)
    # print_vals(dV2, 'dV2 after Q')
    dV =  dV1 - dV2 # dV
    # print_vals(dV, 'dV')
...
def LP_layers(inputs_bounds, parameter, targets = np.asarray([]).reshape(0,0),
    ...
    # LP loop - Initialize history tracking
    V, M = V0, M0
    # if verbose:   
    #     print('V shape', V.shape)     
    #     print('M shape', M.shape) 
    #     print('abs V0 at index 63', tf.abs(V)[0][63].numpy())
    #     print('max abs V0 and the corresponding index', tf.reduce_max(tf.abs(V)).numpy(), tf.argmax(tf.reshape(tf.abs(V), [-1])).numpy())
    #     print('max abs M0 and the corresponding index', tf.reduce_max(tf.abs(M)).numpy(), tf.argmax(tf.reshape(tf.abs(M), [-1])).numpy())                
    Loss_mean_history, Loss_std_history = [], []
    grad_history = {'timestep': [], 'dV_norm': [], 'dM_norm': [], 'V_norm': [], 'M_norm': [],
                    'L1_loss': [], 'L2_loss': [], 'L3_loss': [], 'L4_loss': []}
    ...
    for t in range(1, parameter.timestep+1): 
        ...
        if history:
            ...
            # Track gradients and norms per timestep
            grad_history['timestep'].append(t)
            grad_history['dV_norm'].append(float(tf.norm(dV).numpy()))
            grad_history['dM_norm'].append(float(tf.norm(dM).numpy()))
            grad_history['V_norm'].append(float(tf.norm(V).numpy()))
            grad_history['M_norm'].append(float(tf.norm(M).numpy()))
            ...
            if verbose and (np.log10(t) == int(np.log10(t)) \
                            or t/1.0e3 == int(t/1.0e3)):
            # if verbose:
            #     print('max abs V and the corresponding index', tf.reduce_max(tf.abs(V)).numpy(), tf.argmax(tf.abs(tf.reshape((V), [-1]))).numpy())
            #     print('abs V at index 63', tf.abs(V)[0][63].numpy())
            #     print('max abs M and the corresponding index', tf.reduce_max(tf.abs(M)).numpy(), tf.argmax(tf.abs(tf.reshape((M), [-1]))).numpy())
            #     print('max abs dV and the corresponding index', tf.reduce_max(tf.abs(dV)).numpy(), tf.argmax(tf.abs(tf.reshape((dV), [-1]))).numpy())
            #     print('max abs dM and the corresponding index', tf.reduce_max(tf.abs(dM)).numpy(), tf.argmax(tf.abs(tf.reshape((dM), [-1]))).numpy())
            #     print('average abs dV', tf.reduce_mean(tf.abs(dV)).numpy())
            #     print('average abs dM', tf.reduce_mean(tf.abs(dM)).numpy())
            #     print('Max L', tf.reduce_max(L).numpy())
                print('LP-Loss', t, Loss_mean, Loss_std, 'dV_norm:', grad_history['dV_norm'][-1])   
             
    outputs = output_AMN(V, Vin, V0, parameter, verbose=verbose) 

    return outputs, Loss_mean_history, Loss_std_history, grad_history
...
def AMN_LP(parameter, trainable=True, verbose=False):
    ...
    input_dim, output_dim = parameter.X.shape[1], parameter.output_dim
    inputs = Input(shape=(input_dim,))
    outputs, loss_h, loss_std_h, grad_h = LP_layers(inputs, parameter,
                              trainable=trainable,
                              history=False,
                              verbose=verbose)
    # Compile
...
def MM_LP_QP(parameter, LP=True, loss_outfile=None, targets_outfile= None, 
          history=True, verbose=False, get_stats=True):
    ...
    # run LP or QP
    if LP:
        output, Loss_mean, Loss_std, grad_history = LP_layers(inputs, param, targets=targets,
                       trainable=False, history=history, verbose=verbose)
    else:
        output, Loss_mean, Loss_std, grad_history = QP_layers(inputs, param, targets=targets,
                       trainable=False, history=history, verbose=verbose) 
    ...
    if get_stats:
        ...
        return Vf.numpy(), ReturnStats(r2, 0, Loss_mean[-1], Loss_std[-1],
                                    0, 0, 0, 0), grad_history
    else:
        return Vf.numpy(), grad_history
...
def MM_LP(parameter, loss_outfile=None, targets_outfile= None, 
    ...
    return MM_LP_QP(parameter, LP=True, 
                    ...
                    history=history, verbose=verbose, get_stats=get_stats)
...
def RC(parameter, verbose=False):
    ...
    Res_outputs, _, _, _ = Res_layers(Res_inputs, parameter.res,
...
###############################################################################
# Custom Keras Callback for Gradient and Weight Tracking
###############################################################################

class GradientWeightCallback(keras.callbacks.Callback):
    """Custom callback to track gradients and weight norms during training"""
    def __init__(self, X_train, Y_train):
        super().__init__()
        # Convert to tensors if not already
        self.X_train = tf.convert_to_tensor(X_train, dtype=tf.float32) if not isinstance(X_train, tf.Tensor) else X_train
        self.Y_train = tf.convert_to_tensor(Y_train, dtype=tf.float32) if not isinstance(Y_train, tf.Tensor) else Y_train
        self.gradient_history = []
        self.weight_history = []
        
    def on_epoch_end(self, epoch, logs=None):
        # Track weight norms for all layers
        weight_norms = {}
        for i, layer in enumerate(self.model.layers):
            weights = layer.get_weights()
            if len(weights) > 0:
                # Calculate norm for kernel (weights)
                kernel_norm = float(np.linalg.norm(weights[0]))
                weight_norms[f'layer_{i}_{layer.name}_kernel'] = kernel_norm
                # Calculate norm for bias if exists
                if len(weights) > 1:
                    bias_norm = float(np.linalg.norm(weights[1]))
                    weight_norms[f'layer_{i}_{layer.name}_bias'] = bias_norm
        
        self.weight_history.append({'epoch': epoch, **weight_norms})
        
        # Track gradients - use try/except to handle any issues
        try:
            with tf.GradientTape() as tape:
                # Forward pass
                predictions = self.model(self.X_train, training=True)
                # Compute loss using the model's loss function directly
                loss = self.model.loss(self.Y_train, predictions)
            ...
            for i, (grad, var) in enumerate(zip(gradients, self.model.trainable_variables)):
                if grad is not None:
                    ...
                    grad_norms[f'grad_{i}_{var.name}'] = grad_norm
            
            self.gradient_history.append({'epoch': epoch, **grad_norms})
        except Exception as e:
            # If gradient computation fails, just skip it for this epoch
            # This can happen with complex custom losses
            if epoch == 0:
                print(f"Warning: Gradient tracking failed (will continue without gradients): {e}")
            pass
...
def train_model(parameter, Xtrain, Ytrain, Xtest, Ytest, verbose=False):
    ...
    for kiter in range(Niter): # Looping until properly trained
        ...
        # early stopping and gradient tracking callback
        es = EarlyStopping(monitor='val_loss', mode='min',
                           patience=10, verbose=verbose)
        grad_callback = GradientWeightCallback(Xtrain, Ytrain)
        callbacks = [grad_callback, es] if model.es else [grad_callback]
        # fit
        ...
        # Store gradient and weight tracking in history
        history.gradient_history = grad_callback.gradient_history
        history.weight_history = grad_callback.weight_history
...
class Neural_Model:
    ...
    def __init__(self,
        ...
        # Initialize gradient and weight tracking histories
        self.gradient_history = None
        self.weight_history = None
        self.mechanistic_grad_history = None
...
###############################################################################
# Utility Functions for Gradient and Weight History Analysis
###############################################################################

def save_gradient_history(grad_history, filename):
    """Save gradient history dictionary to JSON file"""
    if grad_history is None or len(grad_history) == 0:
        return
    with open(filename, 'w') as f:
        json.dump(grad_history, f, indent=2)
    print(f'Gradient history saved to {filename}')

def save_weight_history(weight_history, filename):
    """Save weight history list to JSON file"""
    if weight_history is None or len(weight_history) == 0:
        return
    with open(filename, 'w') as f:
        json.dump(weight_history, f, indent=2)
    print(f'Weight history saved to {filename}')

def load_gradient_history(filename):
    """Load gradient history from JSON file"""
    with open(filename, 'r') as f:
        return json.load(f)

def plot_mechanistic_gradients(grad_history, model_type='QP', save_path=None, Stats=None):
    """
    Plot gradient norms and loss components from mechanistic solver iterations
    
    Parameters:
    - grad_history: dictionary with timestep tracking data
    - model_type: 'QP' or 'LP' to customize plot labels
    - save_path: optional path to save figure
    """
    if grad_history is None or len(grad_history.get('timestep', [])) == 0:
        print('No gradient history to plot')
        return
    
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    if Stats:
        line_1 = 'R2 = %.4f (+/- %.4f) Constraint = %.4f (+/- %.4f)' % \
                (Stats.train_objective[0], Stats.train_objective[1],
                 Stats.train_loss[0], Stats.train_loss[1])
        line_2 = 'Q2 = %.4f (+/- %.4f) Constraint = %.4f (+/- %.4f)' % \
                (Stats.test_objective[0], Stats.test_objective[1],
                Stats.test_loss[0], Stats.test_loss[1])
        fig.suptitle('Neural Network - Gradient and Weight Norm Evolution\n' +
                     line_1 + '\n' + line_2, fontsize=16)
    else:
        fig.suptitle('Neural Network - Gradient and Weight Norm Evolution', fontsize=16)    
    
    timesteps = grad_history['timestep']
    
    # Plot 1: Gradient norms
    ax1 = axes[0, 0]
    if model_type == 'LP':
        ax1.plot(timesteps, grad_history['dV_norm'], 'b-', label='dV norm', linewidth=2)
        ax1.plot(timesteps, grad_history['dM_norm'], 'r-', label='dM norm', linewidth=2)
    else:  # QP
        ax1.plot(timesteps, grad_history['grad_norm'], 'g-', label='Gradient norm', linewidth=2)
        ax1.plot(timesteps, grad_history['diff_norm'], 'm-', label='Update norm', linewidth=2)
    ax1.set_xlabel('Timestep')
    ax1.set_ylabel('Norm')
    ax1.set_title('Gradient Norms per Timestep')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ax1.set_yscale('log')
    
    # Plot 2: State variable norms
    ax2 = axes[0, 1]
    ax2.plot(timesteps, grad_history['V_norm'], 'b-', label='V norm', linewidth=2)
    if 'M_norm' in grad_history:
        ax2.plot(timesteps, grad_history['M_norm'], 'r-', label='M norm', linewidth=2)
    ax2.set_xlabel('Timestep')
    ax2.set_ylabel('Norm')
    ax2.set_title('State Variable Norms per Timestep')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    # Plot 3: Individual loss components
    ax3 = axes[1, 0]
    ax3.plot(timesteps, grad_history['L1_loss'], 'b-', label='L1 (Vout)', linewidth=2)
    ax3.plot(timesteps, grad_history['L2_loss'], 'r-', label='L2 (SV)', linewidth=2)
    ax3.plot(timesteps, grad_history['L3_loss'], 'g-', label='L3 (Vin)', linewidth=2)
    ax3.plot(timesteps, grad_history['L4_loss'], 'm-', label='L4 (V≥0)', linewidth=2)
    ax3.set_xlabel('Timestep')
    ax3.set_ylabel('Loss')
    ax3.set_title('Individual Loss Components')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    ax3.set_yscale('log')
    
    # Plot 4: Loss breakdown stacked area
    ax4 = axes[1, 1]
    ax4.stackplot(timesteps, 
                  grad_history['L1_loss'],
                  grad_history['L2_loss'],
                  grad_history['L3_loss'],
                  grad_history['L4_loss'],
                  labels=['L1 (Vout)', 'L2 (SV)', 'L3 (Vin)', 'L4 (V≥0)'],
                  alpha=0.7)
    ax4.set_xlabel('Timestep')
    ax4.set_ylabel('Loss')
    ax4.set_title('Loss Component Breakdown (Stacked)')
...
def plot_nn_gradients(gradient_history, weight_history, save_path=None, Stats=None):
    """
    Plot neural network layer gradients and weight norms over epochs
    
    Parameters:
    - gradient_history: list of dictionaries with epoch gradient data
    - weight_history: list of dictionaries with epoch weight data
    - save_path: optional path to save figure
    """
    if not gradient_history or not weight_history:
        print('No NN gradient/weight history to plot')
        return
    
    # Extract epochs
    epochs = [h['epoch'] for h in gradient_history]
    
    # Get all gradient keys (excluding 'epoch')
    grad_keys = [k for k in gradient_history[0].keys() if k != 'epoch']
    weight_keys = [k for k in weight_history[0].keys() if k != 'epoch']
    ...
    for key in grad_keys:
        values = [h[key] for h in gradient_history]
        ax1.plot(epochs, values, label=key, linewidth=2, alpha=0.7)
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('Gradient Norm')
    ax1.set_title('Gradient Norms per Layer over Training Epochs')
    ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=8)
    ax1.grid(True, alpha=0.3)
    ax1.set_yscale('log')
    
    # Plot 2: Weight norms over epochs
    ax2 = axes[1]
    for key in weight_keys:
        values = [h[key] for h in weight_history]
        ax2.plot(epochs, values, label=key, linewidth=2, alpha=0.7)
    ax2.set_xlabel('Epoch')
    ax2.set_ylabel('Weight Norm')
    ax2.set_title('Weight Norms per Layer over Training Epochs')
...
def plot_combined_history(keras_history, mechanistic_grad_history, model_type='QP', save_path=None, Stats=None):
    """
    Combined plot showing both Keras training history and mechanistic solver gradients
    
    Parameters:
    - keras_history: Keras History object from model.fit()
    - mechanistic_grad_history: gradient history from LP/QP solver
    - model_type: 'QP' or 'LP'
    - save_path: optional path to save figure
    """
    ...
    # Plot 1: Keras training/validation loss
    ...
    if hasattr(keras_history, 'history'):
        ...
        ax1.set_xlabel('Epoch')
        ax1.set_ylabel('Loss')
        ax1.set_title('Keras Training History')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
    
    # Plot 2: Keras metrics
    ax2 = fig.add_subplot(gs[0, 1])
    if hasattr(keras_history, 'history'):
        history_dict = keras_history.history
        metric_key = [k for k in history_dict.keys() if k not in ['loss', 'val_loss']][0] if len([k for k in history_dict.keys() if k not in ['loss', 'val_loss']]) > 0 else None
        if metric_key:
            ax2.plot(epochs, history_dict[metric_key], 'g-', label=f'Training {metric_key}', linewidth=2)
            val_metric_key = f'val_{metric_key}'
            if val_metric_key in history_dict:
                ax2.plot(epochs, history_dict[val_metric_key], 'm-', label=f'Validation {metric_key}', linewidth=2)
            ax2.set_xlabel('Epoch')
            ax2.set_ylabel(metric_key)
            ax2.set_title('Keras Metrics')
            ax2.legend()
            ax2.grid(True, alpha=0.3)
    
    # Plots 3-6: Mechanistic solver gradients
    if mechanistic_grad_history and len(mechanistic_grad_history.get('timestep', [])) > 0:
        timesteps = mechanistic_grad_history['timestep']
        
        # Plot 3: Gradient norms
        ax3 = fig.add_subplot(gs[1, 0])
        if model_type == 'LP':
            ax3.plot(timesteps, mechanistic_grad_history['dV_norm'], 'b-', label='dV norm', linewidth=2)
            ax3.plot(timesteps, mechanistic_grad_history['dM_norm'], 'r-', label='dM norm', linewidth=2)
        else:
            ax3.plot(timesteps, mechanistic_grad_history['grad_norm'], 'g-', label='Gradient norm', linewidth=2)
        ax3.set_xlabel('Timestep')
        ax3.set_ylabel('Norm (log scale)')
        ax3.set_title('Mechanistic Solver - Gradient Norms')
        ax3.legend()
        ax3.grid(True, alpha=0.3)
        ax3.set_yscale('log')
        ...
        ax4.plot(timesteps, mechanistic_grad_history['V_norm'], 'b-', label='V norm', linewidth=2)
        if 'M_norm' in mechanistic_grad_history:
            ax4.plot(timesteps, mechanistic_grad_history['M_norm'], 'r-', label='M norm', linewidth=2)
        ax4.set_xlabel('Timestep')
        ax4.set_ylabel('Norm')
        ax4.set_title('Mechanistic Solver - State Variable Norms')
        ax4.legend()
        ax4.grid(True, alpha=0.3)
        
        # Plot 5: Loss components
        ax5 = fig.add_subplot(gs[2, :])
        ax5.plot(timesteps, mechanistic_grad_history['L1_loss'], 'b-', label='L1 (Vout)', linewidth=2)
        ax5.plot(timesteps, mechanistic_grad_history['L2_loss'], 'r-', label='L2 (SV)', linewidth=2)
        ax5.plot(timesteps, mechanistic_grad_history['L3_loss'], 'g-', label='L3 (Vin)', linewidth=2)
        ax5.plot(timesteps, mechanistic_grad_history['L4_loss'], 'm-', label='L4 (V≥0)', linewidth=2)
        ax5.set_xlabel('Timestep')
        ax5.set_ylabel('Loss (log scale)')
        ax5.set_title('Mechanistic Solver - Individual Loss Components')
        ax5.legend()
